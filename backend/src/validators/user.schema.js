...
export const createBody = z.object({
  name: trimmed(120), work_email: email, password: z.string().min(10, 'Use at least 10 characters').max(200),
  role: z.enum(ROLES).default('EMPLOYEE'), roles: z.array(z.enum(ROLES)).min(1).max(5).optional(),
  employee_id: uuid.optional(),
  // the service enforces 10 characters, so the schema must not accept what it will reject; and there is no
  // in-app self-service change screen, so a new user is not created with a flag nobody can clear
  must_change_pw: z.boolean().default(false),
});
